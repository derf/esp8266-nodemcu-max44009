...
function push_data()
	...
	if lx == nil then
		...
		local json_str = string.format('{"illuminance_lx": %f, "rssi_dbm": %d}', lx, wifi.sta.getrssi())
...
function publish_influx(payload)
	if not publishing_http then
		...
		http.post(influx_url, influx_header, "max44009" .. influx_attr .. " " .. payload, function(code, data)
			publishing_http = false
			gpio.write(ledpin, 1)
			collectgarbage()
		end)
...
function hass_register()
	local hass_device = string.format('{"connections":[["mac","%s"]],"identifiers":["%s"],"model":"ESP8266 + MAX44009","name":"MAX44009 %s","manufacturer":"derf"}', wifi.sta.getmac(), device_id, chip_id)
